...
def main():
    ...
    if username and password and host and port and mydatabase:
        try:
            ...
            examples = [
                {"input": "List all movies with a rating higher than 8.5.",
                 "query": "SELECT * FROM Movies WHERE Rating > 8.5;"},
                {"input": "List all genres available in the database.", "query": "SELECT * FROM Genres;"},
                {"input": "Find all movies released in the year 2020.",
                 "query": "SELECT * FROM Movies WHERE ReleaseYear = 2020;"},
